...
<div class="page-content">
    <div class="row">
        ...
        <br>
        <button class="btn btn-info" id="gritter-sticky" onclick="requestList()">查询采购合同</button>
        <button class="btn btn-info" id="gritter-sticky" onclick="deleteSelected()">删除选定合同</button>
        <button class="btn btn-info" id="gritter-sticky" onclick="uploadContractList()">推送采购合同</button>
    </div>
    <div class="hr hr-16 hr-dotted"></div>
    <div class="row">
        <table id="purchaseContractListTable" class="table table-bordered table-condensed" width="100%">
            <thead>
            <tr>
...
<script type="text/javascript">
    ...
    function requestList(){
        ...
        var table = $("#purchaseContractListTable");
        ...
        table.DataTable({
            ...
            "ajax":{
                "type":"POST",
                "url":"${ctx}/showPurchaseContractList",
...
//      年 月 日 时 分 秒
    ...
    function  tableSelectList() {
        var fileIDList=[];
        for(var i=0;i<$('#purchaseContractListTable').DataTable().rows().count();i++)
        {
            var curnode=$($('#purchaseContractListTable').DataTable().row(i).node());
            if(curnode.find("input[name='singlecheck']").prop('checked'))
            {
                fileIDList.push($('#purchaseContractListTable').DataTable().row(i).data().contractid);
            }
        }
        return fileIDList;
    }
    ...
    function deleteSelected(){
        ...
        $.ajax({
            type: "POST",
            url: "${ctx}/delete/purchaseContract",
    ...
    function uploadContractList(){
        ...
        $.ajax({
            type: "POST",
            url: "${ctx}/uploadPurchaseContractList",
            data:{"fileIDList":fileIDList.toString()},
            success:function (data) {
                alert(data);
                //上传成功后更新页面列表
